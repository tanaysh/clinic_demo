...
Doorkeeper.configure do
  ...
  resource_owner_from_credentials do |_routes|
    ...
    if user.present? && user && user.valid_for_authentication? { user.valid_password?(params[:password]) }
      raise Doorkeeper::Errors::DoorkeeperError, I18n.t('doorkeeper.errors.verify_account') unless user.confirmed?
      user
    else
      raise Doorkeeper::Errors::DoorkeeperError, 'Email and Password did not match.'
    end
